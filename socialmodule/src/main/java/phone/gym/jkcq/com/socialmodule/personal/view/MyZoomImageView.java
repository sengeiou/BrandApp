        ...
        public boolean onTouch(View v, MotionEvent event) {
            ...
            switch (event.getAction() & MotionEvent.ACTION_MASK) {// 单点监听和多点触碰监听
                ...
                case MotionEvent.ACTION_DOWN:
                    ...
                    matrix.set(currentMatrix);
                    makeImageViewFit();
                    //LogUtil.log("zoom img down--------");
                    break;
                // 手指在屏幕上移动，改事件会被不断触发
                case MotionEvent.ACTION_MOVE:
                    //LogUtil.log("zoom img move--------");
                    // 拖拉图片
                    if (mode == MODE_DRAG) {
                ...
                case MotionEvent.ACTION_UP:
                    //LogUtil.log("zoom img up--------");
                    setDoubleTouchEvent(event);

                case MotionEvent.ACTION_POINTER_UP:
                    // System.out.println("ACTION_POINTER_UP");
                    //LogUtil.log("zoom img pointer up--------");
                    mode = 0;
                    // matrix.set(currentMatrix);
                    float[] values = new float[9];
                    matrix.getValues(values);
        ...
        if (currentTime - firstTouchTime >= intervalTime) {
            ...
            if (Math.abs(event.getX() - firstPointF.x) < 30f && Math.abs(event.getY() - firstPointF.y) < 30f) {
                ...
                if (values[Matrix.MSCALE_X] < mMaxScale) {
                    matrix.postScale(mMaxScale / values[Matrix.MSCALE_X], mMaxScale / values[Matrix.MSCALE_X],
                            event.getX(), event.getY());
                } else {
                    matrix.postScale(mMinScale / values[Matrix.MSCALE_X], mMinScale / values[Matrix.MSCALE_X],
                            event.getX(), event.getY());
                }
            }
