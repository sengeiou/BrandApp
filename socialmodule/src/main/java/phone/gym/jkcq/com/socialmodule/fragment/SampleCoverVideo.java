...
public class SampleCoverVideo extends StandardGSYVideoPlayer {
     ...
     */
    protected void changeVideoTextureViewShowType() {
    ...
    public void loadCoverImage(String url, int res) {
        ...
        mCoverOriginUrl = url;
        mDefaultRes = res;
        LoadImageUtil.getInstance().loadCover(getContext().getApplicationContext(), url, mCoverImage);
        LoadImageUtil.getInstance().loadCover(getContext().getApplicationContext(), url, iv_cover, getWidth(), getHeight());
    ...
    public GSYBaseVideoPlayer showSmallVideo(Point size, boolean actionBar, boolean statusBar) {
        ...
        EventBus.getDefault().post(new MessageEvent(MessageEvent.video_start));
    ...
    public void startAfterPrepared() {
        ...
        int typ = GSYVideoType.getShowType();


        int height = getCurrentVideoWidth();
        int with = getCurrentVideoHeight();
        Log.e("startAfterPrepared", "height：" + height + "with:" + with + "typ:" + typ + JkConfiguration.sCurrentFragmentTAG);
        if (JkConfiguration.sCurrentFragmentTAG == JkConfiguration.FRAGMENT_COMMUNITY || JkConfiguration.sCurrentPersonal == JkConfiguration.FRAGMENT_COMMUNITY) {
            if (height > 0 && with > 0) {
                if (getCurrentVideoHeight() <= getCurrentVideoWidth() ) {
                    GSYVideoType.setShowType(GSYVideoType.SCREEN_TYPE_DEFAULT);
                    changeVideoTextureViewShowType();
                    //GSYVideoType.setShowType(GSYVideoType.SCREEN_TYPE_FULL);
                } else if (getCurrentVideoHeight() > getCurrentVideoWidth() ) {
                    GSYVideoType.setShowType(GSYVideoType.SCREEN_TYPE_FULL);
                    changeVideoTextureViewShowType();
