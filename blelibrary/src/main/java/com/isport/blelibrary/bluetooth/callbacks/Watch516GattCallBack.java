...
public class Watch516GattCallBack extends BaseGattCallback {
    ...
    public Watch516GattCallBack(BluetoothListener bluetoothListener, Context context, BaseDevice baseDevice) {
            ...
            if (characteristic.getUuid().equals(Constants.responceW516UUID)) {
                ...
                if (Constants.IS_DEBUG) {
                    ...
                    saveLog(new StringBuilder(DateUtil.dataToString(new Date(), "HH:mm:ss")).append(" ReceiverCmd ")
                            .append
                                    (stringBuilder.toString()).append(" UUID ").append(characteristic.getUuid().toString().substring(0, 8)));
                    Logger.myLog(" ReceiverCmd " + stringBuilder.toString() + " UUID " + characteristic.getUuid()
...
//                            apUserSetting.setMinHeartRate(Utils.byte2Int(values[12]));
                            Logger.myLog("年 " + Utils.byteArrayToInt(new byte[]{values[4], values[3]}) + " 月 " +
...
//                        0F-FF-01-20-1C-07-00-00-32-30-30-2E-30-30-2E-30-32
                        ...
                        Logger.myLog("锻炼数据时间 == " + Utils.byteArrayToInt(new byte[]{values[4], values[3]}) + " " +
...
//                        08-FF-02-E2-07-0C-0D-0E-0F-10
                        Logger.myLog("获取的实时时间 " + Utils.byteArrayToInt(new byte[]{values[4], values[3]}) + "年" +
                    ...
                    } else if (Utils.byte2Int(values[0]) == 0x12 && Utils.byte2Int(values[1]) == 0xFF && Utils.byte2Int
                            (values[2]) == 0x07) {//12-FF-07
                        //获取闹钟设置
//                        实例:
...
//                        应答：12-FF-07-03-0A-0A-31-32-33-00-00-00-00-00-00-00-00-00-00-00
                        ...
                        ParseData.saveAlarm(tempByte, mBaseDevice);
                    } else if (Utils.byte2Int(values[0]) == 0x0A && Utils.byte2Int(values[1]) == 0xFF && Utils.byte2Int
...
//                            Logger.myLog("无闹钟设置");
//                        } else {
                        Logger.myLog("获取睡眠设置,二进制各个bit值 == " + (Utils.byte2Int(booleanArrayG[7]) == 0 ?
                                "自动睡眠" :
                                "窗口睡眠")
                                + (Utils.byte2Int(booleanArrayG[6]) == 0 ? " 睡眠窗口无提醒 " : " 睡眠窗口提醒 ")
                                + (Utils.byte2Int(booleanArrayG[5]) == 0 ? " 勿扰时间关闭 " : " 勿扰时间开启 ") +
                                " 睡眠检测窗口开始时间= " + Utils.byte2Int(values[4]) + ":" + Utils.byte2Int
                                (values[5]) + " 睡眠检测窗口结束时间= " + Utils.byte2Int(values[6]) + ":" + Utils
                                .byte2Int
                                        (values[7]) + " 勿扰时间开始时间= " + Utils.byte2Int(values[8]) + ":" +
                                Utils
                                        .byte2Int
                                                (values[9]) + " 勿扰时间开始时间= " + Utils.byte2Int(values[10])
                                + ":" +
                                Utils
                                        .byte2Int
                                                (values[11]));
...
//                02 5C 00 00 00 00 00 00
                if (bluetoothListener != null) {
                    bluetoothListener.onRealtimeStepData(Utils.byte2Int(values[1]), Utils.byteArrayToInt(new byte[]{values[7],
                            values[6], values[5], values[4]}), 0, 0);
                }
            } else if (characteristic.getUuid().equals(Constants.receiveDataW516UUID)) {
                final StringBuilder stringBuilder = new StringBuilder(values.length);
                for (byte byteChar : values) {
                ...
                //锻炼数据
//                if (bluetoothListener != null) {
//                    bluetoothListener.onRealtimeStepData(Utils.byteArrayToInt(new byte[]{values[7],
...
//            Logger.myLog("end 去解析>127");
            ParseData.parse24HData(m24HDATA, bluetoothListener, mBaseDevice, mContext);
        } else {
    ...
    public boolean writeTXCharacteristicItem(byte[] data) {
        if (isConnected && data != null) {
            ...
            while (!status && isConnected && mNRFBluetoothGatt != null && mRetry < 3) {
                ...
                mRetry++;
                saveLog(new StringBuilder(DateUtil.dataToString(new Date(), "MM/dd HH:mm:ss") + "Write data status:" + status + ",mRetry:" + mRetry + ",isConnected:" + isConnected));
                if (status) {
                    mSendOk = true;
                }
                if (!mSendOk) {
