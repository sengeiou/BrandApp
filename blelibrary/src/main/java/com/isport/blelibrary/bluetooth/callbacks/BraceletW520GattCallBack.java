...
import com.isport.blelibrary.utils.HandlerCommand;
 ...
 * @描述
 */
public class BraceletW520GattCallBack extends BaseGattCallback {


    private String TAG = BraceletW520GattCallBack.class.getCanonicalName();
    ...
    public BraceletW520GattCallBack(BluetoothListener bluetoothListener, Context context, BaseDevice baseDevice) {
...
//        若设备已经配对：02-FF-00-01
        if (Constants.IS_DEBUG) {
            ...
            /*Logger.myLog(" handleReceive: " + stringBuilder.toString() + " UUID " + characteristic.getUuid()
                    .toString());*/
            ...
            if (characteristic.getUuid().equals(Constants.UUID_SEND_DATA_CHAR)) {
                ...
                if (values != null && values.length >= 2) {
                    ...
                    //0xde0127fb00
                            ...
                            (values[2]) == 0x27 && Utils.byte2Int
                            (values[3]) == 0xfb) {

                        Logger.myLog("查询表盘设置成功");
                        if ((W311BluetoothListener) bluetoothListener != null) {
                            ((W311BluetoothListener) bluetoothListener).onGetWatchFaceMode(values[4]);
                        }
...
//                            }
                        ...
                        Logger.myLog("睡眠设置成功");
                        //TODO 这里需要优化
                        BleSettingObservable.getInstance().successSendCmd(new ResultBean(1, 0));
                    } else if (Utils.byte2Int(values[0]) == 0xDE && Utils.byte2Int(values[1]) == 0x01 && Utils.byte2Int
                            (values[2]) == 0x08 && Utils.byte2Int
                            (values[3]) == 0xED) {//设置手环Display成功 DE+01+08+ED
                        Logger.myLog("设置手环Display成功");
                        BleSettingObservable.getInstance().successSendCmd(new ResultBean(HandlerContans.mSenderGetDisplay, 0));
                    } else if (Utils.byte2Int(values[0]) == 0xDE && Utils.byte2Int(values[1]) == 0x01 && Utils.byte2Int
                        ...
                        Parse311Data.parserSleepInfo(values, mContext, bluetoothListener);
                    } else if (Utils.byte2Int(values[0]) == 0xDE && Utils.byte2Int(values[1]) == 0x01 && Utils.byte2Int
                            (values[2]) == 0x08 && Utils.byte2Int
                        ...
                        if (mBaseDevice.deviceType == 30777) {
                            ParseData.saveRaiseHandW307j(values, mBaseDevice);
                        } else {
...
//                    }
                ...
                if ((values[0] & 0xff) == 0xde && (values[1] & 0xff) == 0x02 && (values[2] & 0xff) == 0x10 && (values[3]
                    ...
                    mCache.clear();
                } else if ((values[0] & 0xff) == 0xde && (values[1] & 0xff) == 0x02 && (values[2] & 0xff) == 0x16 && (values[3]
                        & 0xff) == 0xed) {
                    result.clear();
                    result.addAll(mCache);
                    executorService.execute(new Runnable() {
                        @Override
                        public void run() {
                            Parse311Data.parseSportData(result, bluetoothListener, mBaseDevice, mContext);
                        }
                    });
...
//                                initDataTimer();
                ...
                if (HandlerCommand.handleEndPhoneController(mContext, values)) {
                    Logger.myLog("挂断电话");
                    return;
                }
                if (HandlerCommand.handleTakePhoto(values)) {
