...
import com.isport.blelibrary.bluetooth.callbacks.BraceletW520GattCallBack;
...
public class BraceletW520BleManager extends BaseManager {
    ...
    public void disconnect(boolean reconnect) {
        ...
        if (mGattCallBack != null) {
            ...
            Logger.myLog("去断开连接222222222222222 520" + mGattCallBack.mNRFBluetoothGatt);
     ...
     * Bluetooth4.0连接NRF设备
     */
    public void connectNRF(W520Device baseDevice, boolean isConnectByUser) {
        if (btListener == null) {
            setBTListener(btListener);
        }
        if (mGattCallBack == null) {
            //这样会有bug就是mGattCallBacke的值没有改变
            mGattCallBack = new BraceletW520GattCallBack(bluetoothListener, nrfService, baseDevice);
        } else {
            mGattCallBack.setBaseDevice(baseDevice);
        }
        connectNRF(mGattCallBack, baseDevice, isConnectByUser, 300);
       /* if (bluetoothListener != null) {
            Logger.myLog("connectNRF 重连中");
    ...
    public void getSetting() {
        ...
        if (Constants.wristbandWeather != null) {
            // setWeather(Constants.wristbandWeather.getCondition(), Constants.wristbandWeather.getForecast15Days());
        }
        if (!SyncCacheUtils.getSetting(mContext)) {
            SyncCacheUtils.saveSetting(mContext);
            //显示提醒列表
            sendHandler.sendEmptyMessageDelayed(HandlerContans.mSenderGetDisplay, 1000);
            //闹钟列表
            sendHandler.sendEmptyMessageDelayed(HandlerContans.mSenderGetAlarmList, 1600);
            //久坐提醒列表
            sendHandler.sendEmptyMessageDelayed(HandlerContans.mSenderGetSedentary, 2000);
            sendHandler.sendEmptyMessageDelayed(HandlerContans.mHandlerDeviceWatchFace, 3000);
            sendHandler.sendEmptyMessageDelayed(HandlerContans.mSenderGetRaiseHand, 3500);

            //30777没有心率
            if (mCurrentDevice.deviceType != 30777) {
                sendHandler.sendEmptyMessageDelayed(HandlerContans.mSenderGetDisturb, 4000);
                sendHandler.sendEmptyMessageDelayed(HandlerContans.mSenderGetAutoHeart, 4500);
                sendHandler.sendEmptyMessageDelayed(HandlerContans.mSenderSport, 5500);
            } else {
            }
        } else {
            // bracelet_get_sportReadMemory();
            sendHandler.sendEmptyMessageDelayed(HandlerContans.mSenderSport, 2000);
        }
    }
    ...
    private W311BluetoothListener btListener = new W311BluetoothListener() {
        ...
        public void onSendUserInfo() {
            ...
            if (SyncCacheUtils.getFirstBindW311State(mContext)) {
                ...
                sendHandler.sendEmptyMessageDelayed(HandlerContans.mSenderUserInfo, 600);
                syncHandler.sendEmptyMessageDelayed(HandlerContans.mSyncHandlerSyncOpenReal, 1000);
            } else {
                sendHandler.sendEmptyMessageDelayed(HandlerContans.mSenderUserInfo, times);
                syncHandler.sendEmptyMessageDelayed(HandlerContans.mSyncHandlerSyncOpenReal, 600);
            }
            //send_userInfo();
        }
        ...
        public void onGetWatchFaceMode(int mode) {
            Logger.myLog("onGetWatchFaceMode" + mode + "mCurrentDevice.deviceName:" + mCurrentDevice.deviceName + "String.valueOf(BaseManager.mUserId):" + String.valueOf(BaseManager.mUserId));
            Bracelet_W311_SettingModelAction.saveOrUpdateWatchFaces(mCurrentDevice.deviceName, String.valueOf(BaseManager.mUserId), mode + 1);
            Message message = new Message();
        ...
        public void connected() throws IOException {
            ...
            mHandler.sendEmptyMessageDelayed(HandlerContans.mHandlerConnetSuccessState, 0);
            //连接成功读取版本号 3000
        ...
        public void servicesDiscovered() {
            ...
            enableNotification();
            timeOutHandler.removeMessages(HandlerContans.mNotiyFail);
            timeOutHandler.sendEmptyMessageDelayed(HandlerContans.mNotiyFail, 20000);
        }
        ...
        public void successSleepData() {
            mHandler.post(new Runnable() {
                ...
                public void run() {
                    ...
                    if (!isListenerNull) {
                        for (int i = 0; i < mBleReciveListeners.size(); i++) {
                            mBleReciveListeners.get(i).receiveData(new DeviceGetSuccessResult(mCurrentDevice.deviceName, 1, 0));
        ...
        public void onSuccessWatchFace(int watchIndx) {
           /* Message message = new Message();
            Object[] objects = new Object[1];
            objects[0] = watchIndx+1;
            Logger.myLog("DEVICE_WATCH_FACE watchIndex"+ watchIndx+1);
            message.what = HandlerContans.mWatchWatchFace;
            message.obj = objects;
            mHandler.sendMessageDelayed(message, 50);*/
        }
        ...
        public void onSyncCompte() {
            syncHandler.sendEmptyMessageDelayed(HandlerContans.mSyncHandlerComptely, 500);
        }
            ...
            public void handleMessage(Message msg) {
                ...
                switch (msg.what) {
                    ...
                    case HandlerContans.mHandlerConnetFailState:
                        if (!isListenerNull) {
                            Logger.myLog("520 startAni1 mBleReciveListeners mHandlerConnetFailState:" + mBleReciveListeners.size() + "false");
                            for (int i = 0; i < mBleReciveListeners.size(); i++) {
...
//                                .getAddress());
                        Logger.myLog("520 startAni1 mBleReciveListeners: mHandlerConnetSuccessState" + mBleReciveListeners.size() + "true");
                ...
                public void handleMessage(Message msg) {
                    ...
                    switch (msg.what) {
                        ...
                        case HandlerContans.mSenderGetDisplay:
                            ...
                            break;
                        case HandlerContans.mHandlerDeviceWatchFace:
                            get_WatchFace();
                            break;
                        ...
                        case HandlerContans.mSenderGetDisturb:
                            ...
                            break;
                        case HandlerContans.mSenderSport:
                            bracelet_get_sportReadMemory();
                            break;
                ...
                public void handleMessage(Message msg) {
                    ...
                    switch (msg.what) {
                        ...
                        case HandlerContans.mSyncHandlerComptely:
                            ...
                            if (mGattCallBack.mHeartDataChar != null) {
                                set_SyncHrData();
                            } else {
                                if (!isListenerNull) {
                                    for (int i = 0; i < mBleReciveListeners.size(); i++) {
...
//        sendHandler.sendEmptyMessageDelayed(0x04, 700);//设备暂时是自动睡眠
    ...
    public void setRaiseHand(int type) {
        if (mGattCallBack != null) {
            mGattCallBack.writeTXCharacteristicItem(raiseHand(type));
        }
    }
...
//                String s = new String(bytes);
    ...
    public void set_Watch_Face_modle(int enable) {
        if (mGattCallBack != null) {
            Bracelet_W311_SettingModelAction.saveOrUpdateWatchFaces(mCurrentDevice.deviceName, String.valueOf(BaseManager.mUserId), enable);
            mGattCallBack.writeTXCharacteristicItem(sendWatchFace(enable));
        }
    }
    ...
    public void get_verison_battery() {
        if (mGattCallBack != null) {
            mGattCallBack.writeTXCharacteristicItem(send_getDeviceInfo());
        }
    ...
    public void get_WatchFace() {
        if (mGattCallBack != null) {
            mGattCallBack.writeTXCharacteristicItem(getWatchFace());
        }
    }

    public void setWeather(WristbandData wristbandData, List<WristbandForecast> wristbandForecast) {
        WristbandForecast forecast1, forecast2, forecast3;


        boolean havsData = true;
        int todayWeather = 0;
        //温度单位 0：摄氏度。  1：华氏度 （默认摄氏度）
        int todaytempUnit = 0;
        int nexttempUnit = 0;
        int aftertempUnit = 0;
        ...
        int afterhightTemp = 0;
        int afterlowTemp = 0;
        int afteryaqi = 0;

        int len = wristbandForecast.size();
        if (len >= 3) {
            forecast1 = wristbandForecast.get(0);
            forecast2 = wristbandForecast.get(1);
            forecast3 = wristbandForecast.get(2);
            if (Constants.W520WeatherConfig.containsKey(forecast1.getWeatherId())) {
                todayWeather = Constants.W520WeatherConfig.get(forecast1.getWeatherId());
            } else {
                todayWeather = 0;
            }
            if (Constants.W520WeatherConfig.containsKey(forecast2.getWeatherId())) {
                nextWeather = Constants.W520WeatherConfig.get(forecast2.getWeatherId());
            } else {
                nextWeather = 0;
            }
            if (Constants.W520WeatherConfig.containsKey(forecast3.getWeatherId())) {
                afterWeather = Constants.W520WeatherConfig.get(forecast3.getWeatherId());
            } else {
                afterWeather = 0;
            }

            if (!TextUtils.isEmpty(forecast1.getAqiValue())) {
                todayaqi = DateUtil.getAQI(Integer.parseInt(forecast1.getAqiValue()));
            } else {
                todayaqi = 0;
            }

            if (!TextUtils.isEmpty(forecast2.getAqiValue())) {
                nextaqi = DateUtil.getAQI(Integer.parseInt(forecast2.getAqiValue()));
            } else {
                nextaqi = 0;
            }

            if (!TextUtils.isEmpty(forecast3.getAqiValue())) {
                afteryaqi = DateUtil.getAQI(Integer.parseInt(forecast3.getAqiValue()));
            } else {
                afteryaqi = 0;
            }
            ...
            nexthightTemp = TextUtils.isEmpty(forecast2.getHighTemperature()) ? 0 : Integer.parseInt(forecast2.getHighTemperature());
            nextlowTemp = TextUtils.isEmpty(forecast2.getLowTemperature()) ? 0 : Integer.parseInt(forecast2.getLowTemperature());

            afterhightTemp = TextUtils.isEmpty(forecast3.getHighTemperature()) ? 0 : Integer.parseInt(forecast3.getHighTemperature());
            afterlowTemp = TextUtils.isEmpty(forecast3.getLowTemperature()) ? 0 : Integer.parseInt(forecast3.getLowTemperature());
            Logger.myLog("W520 setWeather:havsData true" + havsData + ",todayWeather:" + todayWeather + ",todaytempUnit:" + todaytempUnit + ",todayhightTemp" + todayhightTemp + ",todaylowTemp:" + todaylowTemp + ":todayaqi," + todayaqi);

            if (mGattCallBack != null) {
                mGattCallBack.writeTXCharacteristicItem(sendWeatherCmd(havsData, todayWeather, todaytempUnit, todayhightTemp, todaylowTemp, todayaqi, nextWeather, nexttempUnit, nexthightTemp, nextlowTemp, nextaqi, afterWeather, aftertempUnit, afterhightTemp, afterlowTemp, afteryaqi));
            }
        } else {
            havsData = false;
            Logger.myLog("W520 setWeather:havsData false" + havsData + ",todayWeather:" + todayWeather + ",todaytempUnit:" + todaytempUnit + ",todayhightTemp" + todayhightTemp + ",todaylowTemp:" + todaylowTemp + ":todayaqi," + todayaqi);
    ...
    public void sendSleepData(AutoSleep autoSleep) {
        if (mGattCallBack != null) {
            mGattCallBack.writeTXCharacteristicItem(send_setAutoSleep(autoSleep));
        }
    }

    public void getSleepData() {
        if (mGattCallBack != null) {
            mGattCallBack.writeTXCharacteristicItem(send_get_sleepdata());
        }
    }
    ...
    public void bracelet_get_sportReadMemory() {
        if (mGattCallBack != null) {
            mGattCallBack.writeTXCharacteristicItem(sendReadMemorySportData());
        }
    }
