 ...
 * 线形图表
 */
public class LineChartPractiseBarView extends View {
    private static final String TAG = LineChartPractiseBarView.class.getSimpleName();
...
//    private int hintColor = Color.RED;
    ...
    public LineChartPractiseBarView(Context context) {
        super(context);
        init(context);
    }

    public LineChartPractiseBarView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }

    public LineChartPractiseBarView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void init(Context context) {
        ...
        shadeColors = new int[]{0x99FFFFFF, 0x66FFFFFF, 0x00FFFFFF};
...
//        initHintPaint(context);
    ...
    private void initMaxOrMinPaint(Context context) {
        ...
        if (mTextPaint == null) {
            ...
            mTextPaint.setColor(context.getResources().getColor(R.color.white));//#F3546A
            mTextBound = new Rect();
            String mText = "023";
    ...
    private void initAxisPaint(Context context) {
        axisPaint = new Paint();
        axisPaint.setColor(mContext.getResources().getColor(R.color.color_practise_line));//#F3546A
    ...
    private void initTextPaint(Context context) {
        ...
        textPaint.setColor(context.getResources().getColor(R.color.white));//#F3546A//common_tips_color
    }
    ...
    private void initLinePaint(Context context) {
        ...
        linePaint.setColor(context.getResources().getColor(R.color.common_practise));//#F3546A
...
//                    mDrawArea.right, mDrawArea.top + mDrawArea.height() / 4 + topMargin / 2);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        getArea();
        linePoints.clear();
        canvas.drawColor(mContext.getResources().getColor(R.color.common_view_color));
        //重置线
        linePath.reset();
...
//        }
    ...
    private void drawXline(Canvas canvas) {
        canvas.drawLine(0, mStartY, mTotalWidth - leftMargin * 2, mStartY, axisPaint);
    }

    private void drawXAxisText(Canvas canvas) {
        //这里设置 x 轴的字一条最多显示3个，大于三个就换行
        int xItem = (int) ((mTotalWidth - leftMargin * 2 - 2 * mStartX) / 4);
        // float starX = (mTotalWidth - leftMargin * 2 + mStartX / 2) / 2;
        canvas.drawText(strStartTime, mStartX - mStartX / 2, mStartY + (textPaint.measureText("24")), textPaint);
        ...
        //canvas.drawLine(starX, mStartY, starX, mStartY - DisplayUtils.dip2px(mContext, 3), axisPaint);
        //canvas.drawLine(starX - xItem, mStartY, starX - xItem, mStartY - DisplayUtils.dip2px(mContext, 3), axisPaint);
        // canvas.drawLine(starX + xItem, mStartY, starX + xItem, mStartY - DisplayUtils.dip2px(mContext, 3), axisPaint);
        // canvas.drawLine(starX - (2 * xItem), mStartY, starX - (2 * xItem), mStartY - DisplayUtils.dip2px(mContext, 3), axisPaint);
        // canvas.drawLine(starX + (2 * xItem), mStartY, starX + (2 * xItem), mStartY - DisplayUtils.dip2px(mContext, 3), axisPaint);
        canvas.drawLine(mStartX, mStartY, mStartX, mStartY - DisplayUtils.dip2px(mContext, 3), axisPaint);
        canvas.drawLine(mTotalWidth - DisplayUtils.dip2px(mContext, 5), mStartY, mTotalWidth - DisplayUtils.dip2px(mContext, 5), mStartY - DisplayUtils.dip2px(mContext, 3), axisPaint);
