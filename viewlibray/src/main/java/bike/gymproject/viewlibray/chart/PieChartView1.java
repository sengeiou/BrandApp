...
public class PieChartView1 extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        canvas.drawCircle(getMeasuredWidth() / 2, getMeasuredWidth() / 2, padding + DisplayUtils.dip2px(mContext, 15), mCirclePaint);
    }
    ...
    public void setData(List<PieModel> pieModelList) {
        if (this.mPieModelList == null) {
            this.mPieModelList = new ArrayList<>();
        }
        this.mPieModelList.clear();
        this.mPieModelList.addAll(pieModelList);
        for (int i = 0; i < mPieModelList.size(); i++) {
