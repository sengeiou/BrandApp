...
public class HeartRateRopeIngView extends View {
     ...
     * 结果值颜色
     */
    private int resultNumColor = 0x101D37;
...
//    private Path mPathShader;
    ...
    public HeartRateRopeIngView(Context context) {
        this(context, null);
    }

    public HeartRateRopeIngView(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public HeartRateRopeIngView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    private void init(Context context) {
        ...
        topMargin = context.getResources().getDimension(R.dimen.dp20);
    ...
    private float mStartY, maxHeight;
...
//                valueAnimator.setDuration(100);
        ...
        if (hrList.size() > 0) {
            if (moveX == width / 2) {
                if (showScaleResult) {
                    ...
                    if (currentValue != 0)
                        canvas.drawLine(width / 2 + scaleGap, mStartY - ((250) / 250.0f * maxHeight), width / 2 + scaleGap, mStartY, lagScalePaint);
                } else {
                    canvas.drawLine(width / 2 + scaleGap, mStartY - ((250) / 250.0f * maxHeight), width / 2 + scaleGap, mStartY, lagScalePaint);
                }
                ...
                if (showScaleResult) {
                    //正在滑动的时候,绘制当前整点高度的line,绘制时去除心率line的高度midScaleStroke,使用smallScaleStroke让人感觉正好贴合这线在走
                    if (currentValue != 0)
                        canvas.drawLine(width / 2 + scaleGap, mStartY - ((250) / 250.0f * maxHeight) - topMargin * 2, width / 2 + scaleGap, mStartY, lagScalePaint);
                } else {
                    canvas.drawLine(width / 2 + scaleGap, mStartY - ((250) / 250.0f * maxHeight) - topMargin * 2, width / 2 + scaleGap, mStartY, lagScalePaint);
                }
                // String text = timesText;
    ...
    private void drawResultText(Canvas canvas, String resultText, int index) {
        ...
        //超出边界不绘制
//        - rulerToResultgap / 2
        //float valuePosition = maxHeight - ((250) / 250.0f * maxHeight) - rulerToResultgap + topMargin / 2 + smallScaleStroke - resultNumRect.height() - DisplayUtils.dip2px(mContext, 10);
        ...
        resultNumPaint.getTextBounds(resultText, 0, resultText.length(), resultNumRect);
        if (moveX == width / 2) {
            // canvas.drawText(resultText, width / 2 - resultNumPaint.measureText(resultText) / 2, resultNumRect.height() - DisplayUtils.dip2px(mContext, 10), //绘制当前刻度结果值
            canvas.drawText(resultText, width / 2 - resultNumPaint.measureText(resultText) / 2, mStartY - ((250) / 250.0f * maxHeight + topMargin * 2), //绘制当前刻度结果值
                    resultNumPaint);
        } else {
            canvas.drawText(resultText, width / 2 + scaleGap - resultNumPaint.measureText(resultText) / 2, mStartY - ((250) / 250.0f * maxHeight + topMargin * 2), //绘制当前刻度结果值
                    resultNumPaint);
        }
