 ...
 * 线形图表
 */
public class LineRecChartView extends View {
    private static final String TAG = LineRecChartView.class.getSimpleName();
...
//    private int hintColor = Color.RED;
    ...
    public LineRecChartView(Context context) {
        super(context);
        init(context);
    }

    public LineRecChartView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }

    public LineRecChartView(Context context, AttributeSet attrs, int defStyleAttr) {
...
//        initHintPaint(context);
    ...
    private void initLinePaint(Context context) {
        ...
        linePaint.setStrokeWidth(DisplayUtils.dip2px(context, 2));
        linePaint.setColor(context.getResources().getColor(R.color.color_practise_line));//#F3546A
...
//        hintPaint.setColor(hintColor);
    ...
    HrlineBean beans = null;


    public void setData(List<LineChartEntity> list, boolean isCurv, int maxHr, int minHr, HrlineBean beans) {
...
//                    mDrawArea.right, mDrawArea.top + mDrawArea.height() / 4 + topMargin / 2);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (beans != null) {
            ...
            isFirstEmptyIndex = -1;
            float top, bottom;
            top = mStartY - beans.maxHr * maxHeight / maxYDivisionValue;
            bottom = mStartY - beans.minHr * maxHeight / maxYDivisionValue;
            canvas.clipRect(0, top, mTotalWidth, bottom, Region.Op.INTERSECT);
            if (mData == null || mData.isEmpty()) return;
            // Logger.e("MyLog == onDraw");
            linePaint.setColor(beans.color);
            drawLines(canvas);
        }
...
//                            lineStart + distance, (mStartY - lineHeight) * percent);
          ...
          //  Log.e("lineRec drawLines1", (int) (mStartY - lineHeight) + ",mData.get(i).yValue" + mData.get(i).yValue + "maxHeight=" + maxHeight + ",maxYDivisionValue=" + maxYDivisionValue + ",maxYValue" + maxYValue);

        }
        if (mData.size() == 1) {
            if (mData.get(0).yValue != 0) {
                //只需要画一个点就行了
                canvas.drawCircle(mStartX, linePoints.get(0).y, DisplayUtils.dip2px(mContext, 2), linePaint);
            }
        } else {
