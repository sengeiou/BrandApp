...
public class W311DeviceDataUtil {
     ...
     */
    @NonNull
    public static synchronized void getBracelet_W311_24HDataModel(WatchHistoryNBean watchHistoryNBean, int dataType) {
        //只存储当前设备Id的数据
        if (TextUtils.isEmpty(AppConfiguration.braceletID)) {
            return;
        }
        if (AppConfiguration.braceletID.equals(watchHistoryNBean.getDeviceId())) {
            Bracelet_W311_24HDataModel watch_w516_24HDataModel = new Bracelet_W311_24HDataModel();
            Bracelet_W311_24HDataModel watch_w516_watch_w516_24HDataModelByDeviceId = Bracelet_W311_24HDataModelAction.findBracelet_HDataModelByDeviceId(TokenUtil.getInstance().getPeopleIdInt(BaseApp.getApp()), watchHistoryNBean.getDateStr(), AppConfiguration.braceletID);
            if (watch_w516_watch_w516_24HDataModelByDeviceId == null) {
                ...
                watch_w516_24HDataModel.setTotalSleepTime(watchHistoryNBean.getTotalSleepTime());
                try {
                    ...
                    watch_w516_24HDataModel.setTotalCalories(TextUtils.isEmpty(watchHistoryNBean.getTotalCalories()) ? 0 : (long)
                            (Float.parseFloat(watchHistoryNBean.getTotalCalories())));
                } catch (Exception e) {
                    watch_w516_24HDataModel.setTotalDistance(0);
                    watch_w516_24HDataModel.setTotalCalories(0);
                } finally {
                    ...
                    ParseData.saveBracelet_W311_24HDataModel(watch_w516_24HDataModel);
                    // Logger.myLog("11111111111getBracelet_W311_24HDataModel watch_w516_watch_w516_24HDataModelByDeviceId 111111111111111111111111：dataType:" + dataType + "::" + JkConfiguration.WatchDataType.STEP + ":::" + watchHistoryNBean.getHeartRateDetailArray());
                ...
                try {
                    step = watchHistoryNBean.getTotalSteps();
                    dis = TextUtils.isEmpty(watchHistoryNBean.getTotalDistance()) ? 0 : Float.parseFloat(watchHistoryNBean.getTotalDistance());
                    cal = TextUtils.isEmpty(watchHistoryNBean.getTotalCalories()) ? 0 : (long) (Float.parseFloat(watchHistoryNBean.getTotalCalories()));
                } catch (Exception e) {
                    cal = 0;
                    dis = 0.0f;
                } finally {
                    if (iscurrentDay) {
                        if (watch_w516_watch_w516_24HDataModelByDeviceId.getTotalSteps() < step) {
                            watch_w516_watch_w516_24HDataModelByDeviceId.setTotalSteps(step);
                        }
                        ...
                        case JkConfiguration.WatchDataType.SLEEP:
                            if (iscurrentDay) {
                                if (TextUtils.isEmpty(watch_w516_watch_w516_24HDataModelByDeviceId.getSleepArray())) {
                                    watch_w516_watch_w516_24HDataModelByDeviceId.setTotalSleepTime(watchHistoryNBean.getTotalSleepTime());
                    ...
                    ParseData.updateBracelet_W311_24HDataModel(watch_w516_watch_w516_24HDataModelByDeviceId);
                }
     ...
     */
    private static void setSleepAndHrState(WatchHistoryNBean watchHistoryNBean, int dataType, Bracelet_W311_24HDataModel watch_w516_24HDataModel) {
