...
public class FragmentNoHrDataNewData extends BaseMVPFragment<FragmentDataView, FragmentDataPresenter> implements
        FragmentDataView, DataAddDeviceHolder.OnAddOnclickLister, DeviceGoalStepView,
        DataHeaderHolder.OnHeadOnclickLister, DataDeviceSportHolder.OnSportItemClickListener, DataDeviceSportHolder.OnSportOnclickListenter, DataScaleHolder.OnScaleItemClickListener, DataScaleHolder.OnScaleOnclickListenter, DataHeartRateHolder.OnHeartRateOnclickListenter, DataHeartRateHolder.OnHeartRateItemClickListener, DataSleepHolder.OnSleepOnclickListenter, DataSleepHolder.OnSleepItemClickListener, W311RealTimeDataView, BindBaseView, ThridMeaageView, HrSettingView, Device24HrView, MainHeadLayout.ViewMainHeadClickLister {

    private SmartRefreshLayout home_refresh;


    private static final String TAG = FragmentNoHrDataNewData.class.getSimpleName();
    //***************************************************已整理***********************************************//
    ...
    private Handler handler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            super.handleMessage(msg);
            Logger.myLog("handleMessage" + msg.what);
            switch (msg.what) {
                case 0x01:
    ...
    public void Event(MessageEvent messageEvent) {
        ...
        switch (messageEvent.getMsg()) {
            ...
            case MessageEvent.update_location:
                if (!isFirstLocation) {
                    weatherPresenter.getWeather(Constants.mLocationLatitude, Constants.mLocationLongitude, Constants.cityName, 814);
            ...
            case MessageEvent.update_step:
                Logger.myLog("MessageEvent.update_step-------" + currentType + "，AppConfiguration.braceletID：" + AppConfiguration.braceletID);
                if (DeviceTypeUtil.isContainW81()) {
                    handler.postDelayed(new Runnable() {
                        @Override
                        public void run() {
            ...
            case MessageEvent.BIND_DEVICE_SUCCESS:
                ...
                Logger.myLog("绑定设备成功 == " + baseDevice.deviceType);
                ...
                switch (mResult.getType()) {
                    ...
                    case IResult.DEVICE_GOAL_STEP:
                        ...
                        try {
                            ...
                            deviceGoalStepPresenter.saveDeviceGoalStep(TokenUtil.getInstance().getPeopleIdInt(BaseApp.getApp()), watchGOALSTEPResult.getMac(), watchGOALSTEPResult.getGoalStep());
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
                        break;
    ...
    protected void initData() {
        ...
        Logger.myLog("isFirst=" + isFirst + "TokenUtil.DEVICE_SYNC_FIRST=" + TokenUtil.DEVICE_SYNC_FIRST);
        if (!isFirst) {
        ...
        adapter = new RefrushAdapter<>(getActivity(), lists, R.layout.item, new DefaultAdapterViewLisenter() {
            ...
            public CustomHolder getHeader(Context context, List lists, int itemID) {
                ...
                dataHeaderHolder.setOnCourseOnclickLister(FragmentNoHrDataNewData.this);
            ...
            public CustomHolder getOxyGenItem(Context context, List lists, int itemID) {
                ...
                dataOxygenDataHolder.setHeartRateItemClickListener(FragmentNoHrDataNewData.this, FragmentNoHrDataNewData.this);
                mFragPresenter.getDevcieOxygenData();
            ...
            public CustomHolder getTempItem(Context context, List lists, int itemID) {
                ...
                dataTempHolder.setHeartRateItemClickListener(FragmentNoHrDataNewData.this, FragmentNoHrDataNewData.this);
                mFragPresenter.getTempData();
                //mFragPresenter.getDevcieOnceHrData();
            ...
            public CustomHolder getOnceHrItem(Context context, List lists, int itemID) {
                ...
                dataOnceHrHolder.setHeartRateItemClickListener(FragmentNoHrDataNewData.this, FragmentNoHrDataNewData.this);
                mFragPresenter.getDevcieOnceHrData();
            ...
            public CustomHolder getBloodPressureItem(Context context, List lists, int itemID) {
                ...
                dataBloodPresureDataHolder.setHeartRateItemClickListener(FragmentNoHrDataNewData.this, FragmentNoHrDataNewData.this);
            ...
            public CustomHolder getExecericeItem(Context context, List lists, int itemID) {
                ...
                dataExerciseDataHolder.setHeartRateItemClickListener(FragmentNoHrDataNewData.this, FragmentNoHrDataNewData.this);
                int currentType = AppSP.getInt(context, AppSP.DEVICE_CURRENTDEVICETYPE, JkConfiguration.DeviceType.WATCH_W516);
                if (AppConfiguration.deviceBeanList != null) {
                    if (DeviceTypeUtil.isContainW55X()) {
                        mFragPresenter.getExerciseTodaySum(JkConfiguration.DeviceType.Watch_W556);
            ...
            public CustomHolder getConect1(Context context, List lists, int itemID) {
                ...
                dataSportHolder.setSpoartItemClickListener(FragmentNoHrDataNewData.this, FragmentNoHrDataNewData.this);
                mFragPresenter.getSportHomeData();
                // refleshSport();
            ...
            public CustomHolder getConectHeartRate(Context context, List lists, int itemID) {
                ...
                dataHeartRateHolder.setHeartRateItemClickListener(FragmentNoHrDataNewData.this, FragmentNoHrDataNewData.this);
            ...
            public CustomHolder getConectSleep(Context context, List lists, int itemID) {
                ...
                dataSleepHolder.setSleepItemClickListener(FragmentNoHrDataNewData.this, FragmentNoHrDataNewData.this);
    ...
    private void initRecycleViewItems() {
        lists = new Vector<>();
        //显示的顺序按照list加入的顺序显示
        lists.add(JkConfiguration.BODY_DEVICE2);//实时测试
     ...
     */
    @Override
    public void onHeartRateStateListenter(boolean state) {
        //判断点击的是什么，是否连接
        if (AppConfiguration.isConnected) {
            //是连接状态,点击的不是当前连接设备，那么就去切换连接,点击的是当前，那么进页面检测
            int currentDeviceType = AppSP.getInt(context, AppSP.DEVICE_CURRENTDEVICETYPE, JkConfiguration.DeviceType.WATCH_W516);
    ...
    public void getDeviceSuccess(HashMap<Integer, DeviceBean> deviceBeanHashMap, boolean show, boolean setWatchDefault, boolean isNet) {
        ...
        for (int deviceTypeI : AppConfiguration.deviceBeanList.keySet()) {
            ...
            Logger.myLog("从DB 获取绑定列表成功" + deviceBean.toString());
        ...
        if (AppConfiguration.deviceBeanList.size() > 0) {
            ...
            for (int deviceTypeI : AppConfiguration.deviceBeanList.keySet()) {
                DeviceBean deviceBean = AppConfiguration.deviceBeanList.get(deviceTypeI);
                //Logger.myLog("successGetDeviceListFormHttp：currentType" + currentType + ",deviceBean:" + deviceBean);
                if (deviceBean.deviceType == currentType) {
                    //说明有手表,设置当前设备为手表
                    hasCurrentType = true;
                    AppSP.putInt(context, AppSP.DEVICE_CURRENTDEVICETYPE, deviceBean.deviceType);
                    Logger.myLog("有对应type的设备 currentDeviceType 00== " + currentType);
            ...
            if (!hasCurrentType && (AppConfiguration.deviceBeanList != null)) {
                ...
                Logger.myLog("没有对应type的设备 currentDeviceType 00== " + currentType + "deviceBean" + deviceBean);
                AppSP.putInt(context, AppSP.DEVICE_CURRENTDEVICETYPE, deviceBean.deviceType);
                AppSP.putString(context, AppSP.DEVICE_CURRENTNAME, deviceBean.deviceID);
                if (deviceBean.deviceType == JkConfiguration.DeviceType.BODYFAT) {
                    AppSP.putString(context, AppSP.SCALE_MAC, deviceBean.mac);
...
//        getMainDataFromDB(show);
        //获取网络数据
        if (isNet) {
            Logger.myLog("isNet:" + isNet + " mFragPresenter.getDeviceList");
        ...
        if (DeviceTypeUtil.isContainW81() || DeviceTypeUtil.isContainW556OrW812B()) {
            if (!lists.contains(JkConfiguration.BODY_OXYGEN)) {
                lists.add(3, JkConfiguration.BODY_OXYGEN);
            }
            if (!lists.contains(JkConfiguration.BODY_BLOODPRESSURE)) {
                lists.add(3, JkConfiguration.BODY_BLOODPRESSURE);
            }
            if (!lists.contains(JkConfiguration.BODY_ONCE_HR)) {
                lists.add(3, JkConfiguration.BODY_ONCE_HR);
            }
        ...
        if (DeviceTypeUtil.isContainW557()) {
            if (!lists.contains(JkConfiguration.BODY_TEMP)) {
                lists.add(3, JkConfiguration.BODY_TEMP);
            }
        } else {
            if (lists.contains(JkConfiguration.BODY_TEMP)) {
        ...
        if (DeviceTypeUtil.isContainW81() || DeviceTypeUtil.isContainW55X()) {
            if (!lists.contains(JkConfiguration.BODY_EXCERICE)) {
                lists.add(lists.size() - 1, JkConfiguration.BODY_EXCERICE);
    ...
    public void successGetMainLastHrDataForDb(HeartRateMainData heartRateMainData) {
        ...
        if (mHeartRateMainData != null) {
            Logger.myLog("mHeartRateMainData == " + mHeartRateMainData.toString());
        }
        refreshHeartRate();
    }
    ...
    public void successGetMainLastOxgenData(OxyInfo info) {
        ...
        Logger.myLog("successGetMainLastOxgenData:" + info + "dataOxygenDataHolder" + dataOxygenDataHolder);
        new Handler(Looper.getMainLooper()).postDelayed(new Runnable() {
            @Override
            public void run() {
                if (dataOxygenDataHolder != null) {
    ...
    public void successGetMainLastOnceHrData(OnceHrInfo info) {
        ...
        Logger.myLog("OnceHrInfo:" + info.getHeartValue());
        new Handler(Looper.getMainLooper()).postDelayed(new Runnable() {
            @Override
            public void run() {
    ...
    private synchronized void connectWatchOrBracelet(boolean isConnectByUser, int deviceType) {
     ...
     /*   if (System.currentTimeMillis() - connectTime < 2000) {
        ...
        Logger.myLog("kkk == " + isConnectByUser + "deviceType:" + deviceType);
        int currentDeviceType = AppSP.getInt(context, AppSP.DEVICE_CURRENTDEVICETYPE, JkConfiguration.DeviceType.WATCH_W516);
        //是连接状态，则不用连接了
        ...
        // Logger.myLog("connectWatchOrBracelet:" + deviceBean);
        if (deviceBean != null) {
            AppSP.putInt(context, AppSP.DEVICE_CURRENTDEVICETYPE, deviceBean.deviceType);
            AppSP.putString(context, AppSP.DEVICE_CURRENTNAME, deviceBean.deviceID);
            if (deviceBean.deviceType == JkConfiguration.DeviceType.BODYFAT) {
                ...
                if (DeviceTypeUtil.isContainW81(deviceBean.deviceType)) {
                    AppSP.putString(context, AppSP.WATCH_MAC, DeviceTypeUtil.getW81Mac(deviceBean.deviceName));
                    Logger.myLog(" DeviceTypeUtil.getW81Mac(deviceBean.deviceName):" + DeviceTypeUtil.getW81Mac(deviceBean.deviceName));
                }
            }
        ...
        Logger.myLog("kkk == mac为空，重新扫描 deviceType == " + deviceType);
        ...
        if (TextUtils.isEmpty(watchMac)) {
            ...
            if (ISportAgent.getInstance().isDeviceStartScan()) {
                Logger.myLog("设备已经开始扫描了不需要再一次进行扫描");
                return;
            }
            Logger.myLog("kkk == mac为空，重新扫描 currentName == " + currentName);
            requestPermission(false, isConnectByUser);
        } else {
            // TODO: 2019/1/12 需要和ios平台适配
            if (watchMac.contains(":")) {
                Logger.myLog("kkk == mac不为空，直连" + watchMac + " currentName == " + currentName);
                mFragPresenter.connectDevice(currentName, watchMac, deviceType, true, isConnectByUser);
                setConnectTimeOut();

            } else {
                Logger.myLog("kkk == mac不为空，但不是真实mac，扫描" + watchMac + " currentName == " + currentName);
...
//                    startActivity(intent);
    ...
    public void deviceConSuccess(BaseDevice baseDevice) {
        ...
        defaultConnectState(true);
        //BleProgressObservable.getInstance().hide();
        Logger.myLog("连接成功" + baseDevice.deviceName + " -ADDRESS- " + baseDevice.address + " deviceType == " + baseDevice.deviceType);
        AppSP.putInt(context, AppSP.DEVICE_CURRENTDEVICETYPE, baseDevice.deviceType);
        switch (baseDevice.deviceType) {
            case JkConfiguration.DeviceType.Brand_W811:
            case JkConfiguration.DeviceType.Brand_W814:
